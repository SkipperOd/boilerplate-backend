...
export class UserRolesPermissionsInputMutationResolver {
    ...
    async createUserRolesPermissions(@Arg("data") userRolesPermission: UserRolesPermissionsInput) {
        const userRolePermissions = new _User_Roles_Permissions();
        userRolePermissions.name = userRolesPermission.name;

        const permissions = await _Permissions.findOne(userRolesPermission.permissionId) as _Permissions
        const userRoles = await _User_Roles.findOne(userRolesPermission.userRoleId) as _User_Roles

        userRolePermissions.userRoles = userRoles
        userRolePermissions.permissions = permissions
     
        await getManager().connection.createEntityManager().save(userRolePermissions)
